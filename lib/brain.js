...
const { createWallet } = require('./crypto/wallet-generator')
...
const { getUnitScale, getZeroConf, getCoinConfig, getDepositUrl, isPrefixRequired } = require('./storage/coin-config')
...
const { isValidAddress, formatAddress } = require('./crypto/address-utils')
const { isNonZeroAmountInvoice, getUrlParamInvoice } = require('./crypto/ln-utils')
...
Brain.prototype.mapCryptoUnitsDisplay = function mapCryptoUnitsDisplay (coins) {
  return _.map(coin => {
    const coinConfig = getCoinConfig(coin.cryptoCode)

    return _.assign(coin, {
      displayCode: coinConfig.displayCode,
      unitScale: coinConfig.unitScale
    })
  })(coins)
}
...
Brain.prototype._start = function _start () {
  ...
  if (printPaperWallet) {
    try {
      ...
      if (!_.isNil(createWallet(this.tx.cryptoCode))) {
...
Brain.prototype._privateWalletPrinting = function _privateWalletPrinting () {
  ...
  return this.printer.checkStatus(deviceConfig.kioskPrinter, STATUS_QUERY_TIMEOUT)
    .then((printerStatus) => {
      ...
      const wallet = createWallet(this.tx.cryptoCode)
...
Brain.prototype._startAddressScan = function _startAddressScan () {
  ...
  const resultCallback = (err, address) => {
    ...
    if (!isPrefixRequired(cryptoCode)) {
      const splitAddress = address.split(':')
      address = splitAddress[1] ?? splitAddress[0]
    }

    if (cryptoCode === 'LN') {
      const invoice = getUrlParamInvoice(address)
      if (invoice) address = invoice
      if (isNonZeroAmountInvoice(address)) return this._invalidAddress(true)
    }

    if (!isValidAddress(address, cryptoCode)) return this._invalidAddress()
    if (err) this.emit('error', err)
...
Brain.prototype._firstBill = function _firstBill () {
  ...
  this.browser().send({
    ...
    action: 'scanned',
    buyerAddress: formatAddress(this.tx.toAddress)
  })
  this.enableBillValidator()
...
Brain.prototype.commitCashOutTx = function commitCashOutTx () {
  return this.updateTx({})
    .then(tx => {
      ...
      const depositUrl = getDepositUrl(tx.cryptoCode, tx.toAddress, amountStr)
      const toAddress = formatAddress(tx.toAddress)
...
Brain.prototype.exceedsZeroConf = function exceedsZeroConf (tx) {
  const zeroConfEnabled = getZeroConf(tx.cryptoCode)

  // coins with no zeroConf support will always ask for auth
  if (!zeroConfEnabled) return true

  return tx.fiat.gte(this.trader.zeroConfLimits[tx.cryptoCode])
}
...
Brain.prototype.toCryptoUnits = function toCryptoUnits (cryptoAtoms, cryptoCode) {
  const unitScale = getUnitScale(cryptoCode)
