...
const { getUnitScale, getZeroConf, getCoinConfig } = require('./coin-config')
...
const { isValidAddress, formatAddress, getDepositUrl } = require('./crypto/address-utils')
...
Brain.prototype.mapCryptoUnitsDisplay = function mapCryptoUnitsDisplay (coins) {
  return _.map(coin => {
    const { displayCode, unitScale } = getCoinConfig(coin.cryptoCode)
...
Brain.prototype.commitCashOutTx = function commitCashOutTx () {
  return this.updateTx({})
    .then(tx => {
      ...
      const coinConfig = getCoinConfig(tx.cryptoCode)
      const depositUrl = getDepositUrl(coinConfig, tx.toAddress, amountStr)
