...
function truncateCrypto (cryptoAtoms, cryptoCode) {
  ...
  const scale = getUnitScale(cryptoCode)
...
const validate = (tx) => {
  const required_fields = ['id', 'direction', 'txVersion']
  const missing_fields = required_fields.filter(field => tx[field] === undefined)
  if (missing_fields.length)
    throw E.InvalidTransactionObject(`transaction is missing fields: ${missing_fields.join(' ')}`)
  return true
}
